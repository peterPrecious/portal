  ...
  public partial class Default : FormBase
  {
   ...
   //   notice.Visible = false;
    ...
    protected void lvTiles_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
      ...
      //    this a NOP account
      //    this a facilitator
      //    there are no learners assigned to this account (yet)
      //    this the first visit/time this fac has signed into this account
      //    this fac has no programs assigned to himself/herself (yet)
      //    all existing NOP ecom purchases contain only single seat purchase(s)

      // was the membLastVisit the same day as the ecomIssued?
      int yy1 = Convert.ToDateTime(me.membLastVisit).Year;
      int mm1 = Convert.ToDateTime(me.membLastVisit).Month;
      int dd1 = Convert.ToDateTime(me.membLastVisit).Day;

      int yy2 = Convert.ToDateTime(ec.ecomIssued).Year;
      int mm2 = Convert.ToDateTime(ec.ecomIssued).Month;
      int dd2 = Convert.ToDateTime(ec.ecomIssued).Day;

      if (
          cu.custChannelNop &&
          me.membLevel == 3 &&
          me.membPrograms is null &&
          (yy1 == yy2 && mm1 == mm2 && dd1 == dd2) // see above
         )
      {
        notice.Visible = true;
      }
